...
export async function getUserStats() {
  try {
    ...
    // Get active count
    const active = await prisma.oUR_USER.count({
      where: { status: "ACTIVE" },
    })

    // Get inactive count
    const inactive = await prisma.oUR_USER.count({
      where: { status: "INACTIVE" },
    })

    // Get role counts
    const admins = await prisma.oUR_USER.count({
      where: { role: "admin" },
    })
    ...
    const departmentsResult = await prisma.oUR_USER.findMany({
      ...
      where: { department: { not: undefined } }, // Fix: Use `not: undefined` instead of `not: null`
      distinct: ["department"],
    })
...
export async function checkEmailExists(email: string, excludeId?: string) {
  try {
    ...
    const count = await prisma.oUR_USER.count({ where })
    return count > 0
  } catch (error) {
  ...
  try {
    ...
    const user = await prisma.oUR_USER.create({
      data: {
        ...
        department: data.department || "", // Fix: Use an empty string if `department` is null or undefined
        status:"ACTIVE",
        role: data.role,
      },
    })
